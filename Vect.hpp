...
public:
  ...
  private:
    void init(size_t size)
    {
      ...
      for (size_t t = 0, rg = 0; t < nthreads; t++, rg += segSize)
        ranges[t] = rg;
  ...
  public:
    ...
    Vect evaluate(const Vect &other, Operator op)
    { //  ratio 1.2 improvement w/8 threads
    ...
    Vect &evaluateMutable(const Vect &other, Operator op)
    { //  ratio 1.2 improvement w/8 threads
      assert(vect->size == other.size);
    ...
    Vect &evaluate(const T c, Operator op)
    { //  ratio 1.2 improvement w/8 threads
...
private:
  ...
  static T kahanSum(const T *v, size_t from, size_t to)
  { // sum vector range usign kahan algorithm
    T s = 0, c = 0;
...
public:
  ...
  Vect(T from, T to, T inc, Lambda lambda)
  {
    ...
    if (sz < szSingle)
      for (size_t i = 0; i < sz; i++)
        data[i] = lambda((T)i * inc);
    else
      ThreadedCalc(*this).apply(from, to, inc, lambda);
  }
  Vect(const Vect &v, Lambda lambda)
  { // Vect<double>vx(0, M_PI, 0.001), vy(vx, sin);
    if (v.size > 0)
    {
  ...
  static Vect rnd(size_t cols, size_t rows)
  { // norm random, matrix mode
    auto size = rows * cols;
    assert(size > 0);
    Vect v(cols, rows);
    if (v.size < szSingle)
  ...
  static Vect seq(size_t size, T inc)
  { // 0,inc, 2*inc, ...(n-1)*inc
    assert(size > 0);
    Vect v(size);
    if (v.size < szSingle)
...
public:
  ...
  Vect sub(size_t from, size_t to)
  { // subvector from,to
  ...
  void random()
  {
    if (size < szSingle)
      ...
      ThreadedCalc(*this).rnd(); //, []() -> T {return (T)rand_r(seedp);});
  }
  ...
  {
    if (size < szSingle)
      std::fill(begin(), end(), value); // faster than foreach
    else
      ThreadedCalc(*this).fill(value);
    return *this;
  }
  ...
  Vect &apply(Lambda lambda)
  { // apply labda func -> mutable, usage: v.apply(sin)
    if (size < szSingle)
      for (auto &d : *this)
        d = lambda(d);
    else
  ...
  Vect &apply(std::function<T()> const &lambda)
  { // usage: v.apply(funcNoArgs)
    if (size < szSingle)
      for (auto &d : *this)
        d = lambda();
    else
  ...
  Vect func(std::function<T()> const &lambda)
  { // usage: func(funcNoArgs)
    Vect v(*this);
    if (size < szSingle)
      for (auto &d : v)
        d = lambda();
  ...
  Vect func(Lambda lambda)
  { // non mutable -> aply on copy
    Vect v(*this);
    if (size < szSingle)
      for (auto &d : v)
        d = lambda(d);
  ...
  Vect filter(std::function<bool(T)> const &lambda)
  { // filter by bool lambda conditional expr.
    Vect v;
    if (size < szSingle)
    {
      for (auto const d : *this)
      ...
      ThreadedCalc(*this).filter(v, lambda);
    return v;
  }

  VectIndex filterIndex(std::function<bool(T)> const &lambda)
  { // indexes of selected items
    ...
    if (size < szSingle)
    {
      ...
      ThreadedCalc(*this).filterIndex(res, lambda);
    return res;
  }
  ...
  size_t locate(T c)
  { // use locate on big enough vectors
    return (size < szSingle) ? stlocate(c) : ThreadedCalc(*this).locate(c);
  }
  size_t bsearch(T c)
  { //  binary search -> this must be sorted, return index if found, -1 if not
  ...
  { // asignment
    if (&other == this)
      return *this; // check for self-assignment to avoid delloc problems
  ...
  inline const T &operator[](const Vect &v) const
  { // vect[v] is vect[v._index]
    assert(v._index >= 0 && v._index < size && "Vect: index error");
    return data[v._index];
  };
  Vect operator[](VectIndex ixs) const
  { // index vect by vector of indexes
    Vect v;
    for (auto ix : ixs)
      v << data[ix];
    return v;
  }
  Vect operator[](std::function<bool(T)> const &lambda)
  { // index vect by vector of indexes
    Vect v;
    if (size < szSingle)
    {
  ...
  Vect &operator+=(const T &c)
  {
    if (c != 0)
    {
      if (size < szSingle)
        ...
        ThreadedCalc(*this).evaluate(c, opADD);
    }
    return *this;
  }
  Vect &operator-=(const T &c)
  {
    if (c != 0)
    {
      if (size < szSingle)
        ...
        ThreadedCalc(*this).evaluate(c, opSUB);
    }
    return *this;
  }
  Vect &operator*=(const T &c)
  {
    if (c != 0)
    {
      if (size < szSingle)
        ...
        ThreadedCalc(*this).evaluate(c, opMUL);
    }
    else
      zero();
    return *this;
  ...
  Vect &operator/=(const T &c)
  {
    if (c != 0)
    {
      if (size < szSingle)
        ...
        ThreadedCalc(*this).evaluate(c, opDIV);
    }
    else
      zero(); // assume  v/=0 -> 0
    return *this;
  ...
  Vect &operator+=(const Vect &other)
  {
    ...
    if (size < szSingle)
      ...
      ThreadedCalc(*this).evaluateMutable(other, opADD);
    return *this;
  }
  Vect &operator-=(const Vect &other)
  {
    ...
    if (size < szSingle)
      ...
      ThreadedCalc(*this).evaluateMutable(other, opSUB);
    return *this;
  }
  Vect &operator*=(const Vect &other)
  {
    ...
    if (size < szSingle)
      ...
      ThreadedCalc(*this).evaluateMutable(other, opMUL);
    return *this;
  }
  Vect &operator/=(const Vect &other)
  {
    ...
    if (size < szSingle)
      ...
      ThreadedCalc(*this).evaluateMutable(other, opDIV);
    return *this;
  }
  ...
  T seqSum_dont_use()
  { // don't use as it generates precission errors -> apply kahan algo.
    T s = 0;
    for (auto d : *this)
      s += d;
  ...
  T stsum()
  { // Single Thread Kahan summation algorithm, sequential sum generates precission errors
  ...
  Vect &stadd(const Vect &v)
  {
    for (auto &d : *this)
      d += v[_index++];
    return *this;
  }
  Vect &stsub(const Vect &v)
  {
    for (auto &d : *this)
      d -= v[_index++];
    return *this;
  }
  Vect &stmul(const Vect &v)
  {
    for (auto &d : *this)
      d *= v[_index++];
    return *this;
  }
  Vect &stdiv(const Vect &v)
  {
    for (auto &d : *this)
      d /= v[_index++];
    return *this;
  }
  Vect &stpow(const Vect &v)
  {
    for (auto &d : *this)
      d = pow(d, v[_index++]);
    return *this;
  ...
  Vect stfunc(Lambda lambda)
  { // non mutable -> return a copy applying func
    Vect v(*this);
    for (auto &d : v)
      d = lambda(d);
    return v;
  }
  Vect &stapply(Lambda lambda)
  { // apply labda func -> mutable, usage: v.apply(sin)
    for (auto &d : *this)
      d = lambda(d);
    return *this;
  }
  Vect &stapply(std::function<T()> const &lambda)
  { // usage: v.apply(funcNoArgs)
    for (auto &d : *this)
      d = lambda();
    return *this;
  ...
  Vect stfilter(std::function<bool(T)> const &lambda)
  { // filter by bool lambda conditional expr.
    Vect v;
    for (auto const d : *this)
      if (lambda(d))
  ...
  VectIndex stfilterIndex(std::function<bool(T)> const &lambda)
  { // indexes of selected items
    VectIndex res;
    for (size_t ix = 0; ix < size; ix++)
      if (lambda(data[ix]))
